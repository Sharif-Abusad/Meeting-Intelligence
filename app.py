...
#             transcript = transcribe_audio(chunks, language)
...
REQUEST_TIMEOUT = 30          # seconds, for quick request/response calls
POLL_INTERVAL_SECONDS = 3     # how often to poll job status
POLL_TIMEOUT_SECONDS = 60 * 30  # give up after 30 minutes
...
def submit_meeting(youtube_url: str | None, uploaded_file, language: str) -> str:
    """
    Submit a job to the API - either a YouTube URL or an uploaded file.
    Returns the job_id.
    """
    data = {"language": language, "build_index": "true"}
    files = None
...
def get_job_status(job_id: str) -> dict:
    try:
        response = requests.get(
            f"{API_BASE_URL}/meetings/{job_id}/status", timeout=REQUEST_TIMEOUT
        )
    except requests.RequestException as exc:
        raise APIError(f"Could not reach the API at {API_BASE_URL}: {exc}") from exc
...
def get_job_result(job_id: str) -> dict:
    try:
        response = requests.get(
            f"{API_BASE_URL}/meetings/{job_id}", timeout=REQUEST_TIMEOUT
        )
    except requests.RequestException as exc:
        raise APIError(f"Could not reach the API at {API_BASE_URL}: {exc}") from exc
...
def ask_meeting_question(job_id: str, question: str) -> str:
    try:
        response = requests.post(
            f"{API_BASE_URL}/meetings/{job_id}/ask",
...
def wait_for_completion(job_id: str, progress_placeholder) -> dict:
    """
    Poll the job status endpoint until it completes or fails, updating
    the given placeholder with elapsed time. Returns the final result dict.
    """
    started = time.time()
    ...
    while True:
        ...
        if elapsed > POLL_TIMEOUT_SECONDS:
            raise APIError("Timed out waiting for the pipeline to finish.")
        ...
        if status == "completed":
            st.session_state.pipeline_steps = {
                k: "done" for k in ["audio", "transcript", "title", "summary", "extract", "rag"]
            }
            return get_job_result(job_id)

        if status == "failed":
            st.session_state.pipeline_steps = {
                k: "pending" for k in ["audio", "transcript", "title", "summary", "extract", "rag"]
            }
            raise APIError(status_payload.get("error") or "Pipeline failed for an unknown reason.")

        # "queued" / "processing" - the API doesn't expose per-stage
        # granularity, so all stages pulse together while we wait.
        st.session_state.pipeline_steps = {
            k: "active" for k in ["audio", "transcript", "title", "summary", "extract", "rag"]
        }
        progress_placeholder.info(f"⚙️ {status.capitalize()}… ({elapsed}s elapsed)")
...
with st.sidebar:
    ...
    if input_mode == "YouTube URL":
        ...
        uploaded_file = st.file_uploader(
            "Audio / video file",
            type=["mp3", "wav", "m4a", "mp4", "mov", "webm", "ogg", "flac", "aac"],
        )
...
# ─── Main Area ──────────────────────────────────────────────────────────────────
st.markdown('<div class="hero-title">AI Video Assistant</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-sub">Transcribe · Summarize · Chat with your meetings</div>', unsafe_allow_html=True)
st.markdown("---")

# ── Run Pipeline ────────────────────────────────────────────────────────────────
if run_btn:
    if input_mode == "YouTube URL" and not (youtube_url and youtube_url.strip()):
        ...
        try:
            with progress_placeholder.container():
                st.info("⚙️ Submitting to pipeline…")

            job_id = submit_meeting(
            ...
            result = wait_for_completion(job_id, progress_placeholder)
...
if st.session_state.result:
    ...
    if send_btn and user_input.strip():
        try:
            with st.spinner("Thinking…"):
                answer = ask_meeting_question(st.session_state.job_id, user_input.strip())
            st.session_state.chat_history.append({"role": "user",      "content": user_input.strip()})
            st.session_state.chat_history.append({"role": "assistant", "content": answer})
            st.rerun()
        except APIError as e:
    ...
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5rem 2rem;text-align:center">
        ...
        <div style="color:var(--text-muted);font-size:0.85rem;max-width:380px;line-height:1.7">
            Choose a YouTube URL or upload a file in the sidebar, choose your language, and hit <strong>Analyse</strong> to get started.
        </div>
        <div style="margin-top:2rem;display:flex;gap:1rem;flex-wrap:wrap;justify-content:center">
            <span class="badge badge-purple">Transcription</span>
